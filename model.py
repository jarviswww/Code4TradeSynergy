...
class ICLoss(nn.Module):
    def __init__(self, dim, batch_size, penalty, penalty_scale, slack, threshold, temperature=0.5):
    ...
    def forward(self, emb_i, emb_j, pos_embeds, target):
        if not self.slack:
            SIZE = emb_i.shape[0]
            ...
            cluster_similarity_matrix = torch.matmul(emb_j, pos_embeds.T)  
            topk_values, topk_indices = torch.topk(cluster_similarity_matrix, k=self.threshold, dim=1)

            slack_positives = pos_embeds[topk_indices]  # shape: (batch_size, threshold, emb_dim)
...
class SessionGraph(Module):
    def __init__(self, opt, n_node, item_2hint, hint_2item, count):
        ...
        self.ICLoss = ICLoss(self.hidden_size, self.batch_size, self.penalty, self.penalty_scale, self.slack, self.threshold, self.temperature)
    ...
    def loss_computation(self, seq, targets):
        ...
        ICLoss = self.ICLoss(seq, target_intent_embed, self.item_centroids, target_intent)
        # -------- CELoss --------- #
        l_c = seq
